...
echo "[sync] 2/5 Wiping dev database (public + drizzle schemas)..."
psql "$DEV_DIRECT" --quiet -c "
  DROP SCHEMA IF EXISTS public CASCADE; CREATE SCHEMA public;
  DROP SCHEMA IF EXISTS drizzle CASCADE;
" 2>&1 | grep -v '^NOTICE\|^DETAIL\|^drop cascades' || true
...
pg_restore \
  ...
  --no-owner \
  --no-privileges \
  "$DUMP_FILE" 2>&1 | grep -v 'schema "public" already exists\|schema "drizzle" already exists\|errors ignored on restore\|Command was:' | sed '/^$/d' || true

echo "[sync] 4/5 Applying new migrations..."
(cd "$PROJECT_ROOT" && npm run db:migrate 2>&1) | grep -E '^\[✓\]|applied successfully' || true

echo "[sync] 5/5 Refreshing query planner statistics..."
